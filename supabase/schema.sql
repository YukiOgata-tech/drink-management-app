...
-- =====================================================
--
-- ⚠️⚠️⚠️ 重要警告: このDBは共有・本番環境です ⚠️⚠️⚠️
-- このスキーマが適用される Supabase プロジェクト（ctwpnaizwsrffrkkbuig /
-- "each-spirit and drink-mgt"）は、each-spirit.com の Web アプリと
-- 1つの Postgres を共有しています。
--   - public スキーマ … 本アプリ（drink-management）専用
--   - es スキーマ     … each-spirit.com 専用（絶対に触らない）
--
-- 下記の DROP TABLE ... CASCADE は「まっさらな新規プロジェクト初期化用」です。
-- 既に本番データ（profiles / drink_logs 等）が入っている共有DBに対しては
-- 【実行しないでください】。public 側の実データが消えます（es 側は無事ですが）。
-- スキーマ変更は schema.sql の再実行ではなく、supabase/migrations/ に
-- 追加マイグレーションを書いて該当 DDL だけを適用する運用にしてください。
-- =====================================================

-- 既存のテーブルがあれば削除（⚠️ 新規プロジェクト初期化時のみ。共有DBでは実行禁止）
...
-- 9. 公式ドリンクDB（products）
-- =====================================================
-- アプリ内のドリンクカタログ（公式登録商品）。
-- lib/products.ts / stores/products.ts から参照される。
-- 全ユーザーが閲覧可能（RLS: SELECT = true）。投入・更新は管理者（service role）のみ。
...
COMMENT ON TABLE public.products IS '公式ドリンクカタログ（ブランド・メーカー・JANコード含む）';
COMMENT ON COLUMN public.products.category IS 'DrinkCategory（types/index.ts の DrinkCategory に対応）';
COMMENT ON COLUMN public.products.is_official IS '公式登録商品フラグ';

-- products は RLS が単純（全員閲覧可・書き込みは service role のみ）なため、
-- 複合的なポリシーを集約している rls.sql ではなくここで完結させる。
...
CREATE POLICY "Products are viewable by everyone"
  ...
  USING (true);
-- INSERT/UPDATE/DELETE ポリシーは無し（= 一般ユーザーは書き込み不可、service role のみ）
