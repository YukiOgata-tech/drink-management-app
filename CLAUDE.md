...
│   │   └── signup.tsx      # 新規登録画面
│   ├── legal/              # 法的ドキュメント
│   │   ├── _layout.tsx     # レイアウト（モーダル表示）
│   │   ├── terms.tsx       # 利用規約
│   │   ├── privacy-policy.tsx # プライバシーポリシー
│   │   └── drinking-guide.tsx # 飲酒ガイドライン
│   └── (tabs)/             # メインタブナビゲーション
│       ├── _layout.tsx     # タブレイアウト
...
- **法的コンプライアンス**: 年齢確認、利用規約、プライバシーポリシー、飲酒ガイドラインを実装

## 法的コンプライアンス機能

### 年齢確認
- 初回起動時に生年月日入力による年齢確認を実施
- 20歳未満のユーザーはアプリを利用不可
- 年齢確認結果はローカルストレージに保存

### 同意管理（`stores/consent.ts`）
- 利用規約、プライバシーポリシーへの明示的同意を取得
- 同意バージョン管理（`types/index.ts`の`LEGAL_VERSIONS`で管理）
- バージョン更新時に再同意を要求可能

### 法的ドキュメント（`app/legal/`）
- **利用規約** (`terms.tsx`): サービス内容、免責事項、禁止行為、準拠法等
- **プライバシーポリシー** (`privacy-policy.tsx`): データ収集・保存・第三者提供・ユーザー権利
- **飲酒ガイドライン** (`drinking-guide.tsx`): 厚生労働省基準、健康リスク、相談窓口

### 飲酒に関する注意事項
- 20歳未満の飲酒禁止（未成年者飲酒禁止法）
- 飲酒運転の禁止（道路交通法）
- 妊娠中・授乳中の飲酒注意
- 適正飲酒量の案内（男性20g/日、女性10g/日）
- 相談窓口情報（よりそいホットライン、精神保健福祉センター等）
...
- ✅ 同意画面（年齢確認、利用規約・プライバシーポリシー同意）
- ✅ 法的ドキュメント（利用規約、プライバシーポリシー、飲酒ガイドライン）
- ✅ ユーザープロフィール管理
